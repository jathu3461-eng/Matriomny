 ...
 * Priority countries (Sri Lankan diaspora destinations) appear first;
 * the remaining 230+ countries are sorted alphabetically.
 *
 * Each entry: { code, name, dialCode, flag }
 *   code     — ISO 3166-1 alpha-2
 *   name     — English display name
 *   dialCode — ITU-T E.164 prefix (string, e.g. "+94")
 *   flag     — emoji flag
 */

export const PRIORITY_CODES = ['LK', 'IN', 'AE', 'AU', 'CA', 'GB', 'US'];

export const COUNTRIES = [
  // ── Priority: Sri Lankan diaspora destinations ──────────────────────────────
  ...
  { code: 'CF', name: 'Central African Rep.',   dialCode: '+236', flag: '🇨🇫' },
...
/** The default country for this Sri Lankan matrimony application. */
export const DEFAULT_COUNTRY = COUNTRIES[0]; // Sri Lanka
...
/**
 * Find the best-matching country from an E.164 dial prefix.
 * Tries longest prefix first to avoid e.g. "+1" winning over "+1868".
 * Returns DEFAULT_COUNTRY if nothing matches.
 */
export function findCountryByDialCode(dialCode) {
  if (!dialCode) return DEFAULT_COUNTRY;
  const sorted = [...COUNTRIES].sort((a, b) => b.dialCode.length - a.dialCode.length);
  return sorted.find((c) => dialCode.startsWith(c.dialCode)) ?? DEFAULT_COUNTRY;
}
